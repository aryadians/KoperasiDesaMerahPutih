...
<div class="split-layout">
    ...
    <div class="main-column">
        @if($activeLoan)
            <div class="standard-card">
                <h3 style="font-size: 18px; font-weight: 600; margin-bottom: 16px; border-bottom: 1px solid var(--hairline); padding-bottom: 12px; display: flex; justify-content: space-between; align-items: center;">
                    <span>Pinjaman Aktif Anda</span>
                    <span style="font-size: 12px; font-weight: 600; text-transform: uppercase; padding: 4px 10px; border-radius: var(--r-full);
                ...
                <div style="display: grid; grid-template-columns: repeat(2, 1fr); gap: 16px; margin-bottom: 24px;">
                    <div>
                        <span style="font-size: 12px; color: var(--muted);">Kode Kontrak</span>
                        <div style="font-size: 16px; font-weight: 600; margin-top: 4px;">{{ $activeLoan->loan_code }}</div>
                    </div>
                    <div>
                        <span style="font-size: 12px; color: var(--muted);">Tenor Pinjaman</span>
                        <div style="font-size: 16px; font-weight: 600; margin-top: 4px;">{{ $activeLoan->tenor_months }} Bulan</div>
                    </div>
                    <div>
                        <span style="font-size: 12px; color: var(--muted);">Nominal Diajukan</span>
                        <div style="font-size: 16px; font-weight: 600; margin-top: 4px;">Rp {{ number_format($activeLoan->amount_requested, 0, ',', '.') }}</div>
                    </div>
                    <div>
                        <span style="font-size: 12px; color: var(--muted);">Nominal Disetujui</span>
                        <div style="font-size: 16px; font-weight: 600; margin-top: 4px; color: var(--primary);">
                            Rp {{ number_format($activeLoan->amount_approved, 0, ',', '.') }}
                        </div>
                    </div>
                    <div>
                        <span style="font-size: 12px; color: var(--muted);">Suku Bunga Koperasi</span>
                        <div style="font-size: 16px; font-weight: 600; margin-top: 4px;">{{ $activeLoan->interest_rate }}% Flat</div>
                    </div>
                    <div>
                        <span style="font-size: 12px; color: var(--muted);">Total Pengembalian</span>
                        @php 
                ...
                @if($activeLoan->status === 'active')
                    ...
                    @if($activeLoan->payments->isEmpty())
                        <div style="padding: 16px; text-align: center; border: 1px dashed var(--hairline); border-radius: var(--r-sm); color: var(--muted); font-size: 14px;">
                    ...
                    @endif
                @else
                    <div style="background-color: var(--surface); padding: 16px; border-radius: var(--r-sm); font-size: 13px; color: var(--muted); text-align: center;">
                        Menunggu verifikasi administrasi oleh pengurus koperasi untuk pencairan dana pinjaman.
                    </div>
                @endif
            </div>
        @else
            <div style="text-align: center; padding: 48px; border: 1px dashed var(--hairline); border-radius: var(--r-md); color: var(--muted);">
                <span style="font-size: 32px;">📄</span>
                <p style="font-size: 15px; margin-top: 12px;">Anda tidak memiliki pengajuan pinjaman aktif saat ini.</p>
                <p style="font-size: 13px; color: var(--muted); margin-top: 4px;">Isi formulir di sebelah kanan untuk mengajukan pinjaman modal usaha kecil.</p>
            </div>
        @endif

        <!-- Loan History -->
        <div class="standard-card" style="margin-top: 24px; padding: 0; overflow: hidden;">
            <h3 style="font-size: 16px; font-weight: 600; padding: 20px; border-bottom: 1px solid var(--hairline);">Riwayat Seluruh Pinjaman</h3>
            @if($loans->isEmpty())
    ...
    <div class="sticky-rail">
        <div class="reservation-card">
            <h3 style="font-size: 18px; font-weight: 600; border-bottom: 1px solid var(--hairline); padding-bottom: 12px;">Ajukan Pinjaman</h3>
            
            @if($activeLoan)
                <div style="padding: 16px; background-color: #ffebeb; color: var(--danger); border-radius: var(--r-sm); font-size: 13px; font-weight: 500; line-height: 1.5;">
            ...
            @else
                <form action="{{ route('member.loans.apply') }}" method="POST">
                    ...
                    <div style="background-color: var(--surface); padding: 12px; border-radius: var(--r-sm); font-size: 13px; color: var(--body); line-height: 1.5; margin-bottom: 20px;">
